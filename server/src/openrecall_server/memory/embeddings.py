...
``/embeddings`` endpoint, local (Ollama, mlx) or cloud, chosen by OPENRECALL_EMBED_*
...
class OpenAICompatibleEmbedder:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "OpenAICompatibleEmbedder":
        model = env.get("OPENRECALL_EMBED_MODEL")
        if not model:
            raise ValueError("OPENRECALL_EMBED_MODEL is required (the embedding model name)")
        return cls(
            base_url=env.get("OPENRECALL_EMBED_BASE_URL", DEFAULT_BASE_URL),
            model=model,
            api_key=env.get("OPENRECALL_EMBED_API_KEY"),
        )
