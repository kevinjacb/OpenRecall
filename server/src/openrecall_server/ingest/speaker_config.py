...
One frozen seam for every ``OPENRECALL_SPEAKER_*`` number. Loaded once at startup; a
...
ENV_ENABLED = "OPENRECALL_SPEAKER_ENABLED"
...
def load_speaker_config(env: Mapping[str, str]) -> SpeakerConfig:
    ...
    if "OPENRECALL_SPEAKER_EMBED_MODEL" in env:
        kwargs["embed_model"] = env["OPENRECALL_SPEAKER_EMBED_MODEL"]
    if "OPENRECALL_SPEAKER_EMBED_BASE_URL" in env:
        kwargs["embed_base_url"] = env["OPENRECALL_SPEAKER_EMBED_BASE_URL"]
    if "OPENRECALL_SPEAKER_EMBED_API_KEY" in env:
        kwargs["embed_api_key"] = env["OPENRECALL_SPEAKER_EMBED_API_KEY"]
    for f in (
        "confirm_threshold", "tentative_threshold", "cluster_threshold",
        "ema_alpha", "outlier_trim_pct", "min_confidence",
    ):
        envn = f"OPENRECALL_SPEAKER_{f.upper()}"
        if envn in env:
