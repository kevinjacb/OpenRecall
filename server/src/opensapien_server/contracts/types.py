...
class Provenance(BaseModel):
    ...
    Produced via :meth:`opensapien_server.memory.atom.MemoryAtom.to_provenance`.
...
class IssueCommandPayload(BaseModel):
    ...
    a persisted :class:`~opensapien_server.commands.model.Command`.
    """
...
class PlannerOutcome(str, Enum):
    ...
    :class:`~opensapien_server.http.routes.dto.AgentResponseDTO`.
...
class PlannerContext(BaseModel):
    ...
    ISSUE_COMMAND prohibition (see :mod:`opensapien_server.agent.planner`).
    """
...
class PlannerResult(BaseModel):
    ...
    # :data:`~opensapien_server.http.routes.dto.MAX_ATOM_CHIP_TEXT_LEN`.
    atoms: tuple[ScoredAtom, ...] = Field(default_factory=tuple)
