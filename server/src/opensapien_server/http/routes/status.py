...
def _version() -> str:
    """The package version, via `__version__` (set in src/opensapien_server/__init__.py).
    ...
    is available.
    """
    from opensapien_server import __version__ as v
    if isinstance(v, str) and v:
        return v
    try:
        from importlib.metadata import version
        return version("opensapien-server")
