...
chosen by OPENSAPIEN_VLM_* config. No vision model is hardcoded. Wire shape and config
...
class OpenAICompatibleVisionModel:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "OpenAICompatibleVisionModel":
        model = env.get("OPENSAPIEN_VLM_MODEL")
        if not model:
            raise ValueError("OPENSAPIEN_VLM_MODEL is required (the vision model name)")
        return cls(
            base_url=env.get("OPENSAPIEN_VLM_BASE_URL", DEFAULT_BASE_URL),
            model=model,
            api_key=env.get("OPENSAPIEN_VLM_API_KEY"),
        )
