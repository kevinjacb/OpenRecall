...
One concrete :class:`~opensapien_server.memory.extract.ChatModel` that talks to *any*
...
class OpenAICompatibleChatModel:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "OpenAICompatibleChatModel":
        """Build from environment: OPENSAPIEN_LLM_{BASE_URL,MODEL,API_KEY,TEMPERATURE}."""
        model = env.get("OPENSAPIEN_LLM_MODEL")
        if not model:
            raise ValueError("OPENSAPIEN_LLM_MODEL is required (the model name to use)")
        return cls(
            base_url=env.get("OPENSAPIEN_LLM_BASE_URL", DEFAULT_BASE_URL),
            model=model,
            api_key=env.get("OPENSAPIEN_LLM_API_KEY"),
            temperature=float(env.get("OPENSAPIEN_LLM_TEMPERATURE", "0.2")),
        )
