...
``/embeddings`` endpoint, local (Ollama, mlx) or cloud, chosen by OPENSAPIEN_EMBED_*
...
class OpenAICompatibleEmbedder:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "OpenAICompatibleEmbedder":
        model = env.get("OPENSAPIEN_EMBED_MODEL")
        if not model:
            raise ValueError("OPENSAPIEN_EMBED_MODEL is required (the embedding model name)")
        return cls(
            base_url=env.get("OPENSAPIEN_EMBED_BASE_URL", DEFAULT_BASE_URL),
            model=model,
            api_key=env.get("OPENSAPIEN_EMBED_API_KEY"),
        )
