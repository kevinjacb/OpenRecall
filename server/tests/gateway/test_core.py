...
def _core_with_speaker():
    ...
    reg.add_speaker(Speaker(
        ...
        enrollment_status="implicit", centroid=unit, embedding_model="m",
        dim=4, turn_count=0, first_seen="2026-07-26T00:00:00+00:00",
        updated_at="2026-07-26T00:00:00+00:00"))
    reg.add_speaker(Speaker(
        speaker_id="b", display_name=None, is_wearer=False,
        enrollment_status="confirmed", centroid=unit, embedding_model="m",
        dim=4, turn_count=0, first_seen="2026-07-26T00:00:00+00:00",
        updated_at="2026-07-26T00:00:00+00:00"))
    ...
    from sense_server.gateway.core import GatewayCore
    core = GatewayCore(
        pipeline_factory=lambda start_seq: AudioIngestPipeline(
            reassembler=SessionReassembler(start_seq=start_seq),
            decoder=FakeDecoder(), transcriber=FakeTranscriber(),
            hop_ms=20, window_ms=100, sample_rate=16000),
        event_store=store, speaker_registry=reg, atom_store=atoms,
    )
    return core, reg, store, atoms


def test_name_speaker_control_calls_registry_name():
    from sense_server.protocol.messages import NameSpeaker

    core, reg, _store, _atoms = _core_with_speaker()
    out = core.on_control(NameSpeaker(
        session_id="s1", speaker_id="a", name="Sarah"))
    assert out == []  # no outbound frames
    s = reg.get("a")
    assert s.display_name == "Sarah"
    assert s.enrollment_status == "confirmed"


def test_reassign_speaker_control_rel_labels_events_and_atoms():
    ...
    core, _reg, store, atoms = _core_with_speaker()
    for seq in range(3):
    ...
    atoms.append(MemoryAtom(
        atom_id="m1", session_id="s1", source_event_id="s1:0", kind="fact",
        text="y", created_at=datetime(2026, 7, 26, tzinfo=timezone.utc),
        start_ms=0, speaker="a", speaker_confidence=0.9,
        speaker_assignment="confirmed"))

    out = core.on_control(ReassignSpeaker(
        session_id="s1", from_speaker_id="a", to_speaker_id="b", scope="all"))
    assert out == []
    assert all(e.speaker == "b" for e in store.events("s1"))
    assert atoms.atoms("s1")[0].speaker == "b"
