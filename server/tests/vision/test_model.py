...
(Qwen2.5-VL via mlx_vlm/Ollama) or cloud — chosen by OPENRECALL_VLM_* config. The HTTP
...
def test_from_env_requires_a_model():
    with pytest.raises(ValueError):
        OpenAICompatibleVisionModel.from_env({"OPENRECALL_VLM_BASE_URL": "http://x/v1"})


def test_from_env_reads_config():
    m = OpenAICompatibleVisionModel.from_env(
        {
            "OPENRECALL_VLM_BASE_URL": "http://localhost:8080/v1",
            "OPENRECALL_VLM_MODEL": "qwen2.5-vl",
            "OPENRECALL_VLM_API_KEY": "k",
        }
    )
