...
class SenseLogTest {
    ...
    fun `format omits optional fields when null`() {
        val out = SenseLog.format("hi", TraceContext("req-1"))
    ...
    fun `format passes through msg unchanged when no trace`() {
        val out = SenseLog.format("plain", null)
