package com.opensapien.relay.arch
 ...
 * anything from `com.opensapien.relay.http` or `com.opensapien.relay.http.dto`.
 *
...
class ArchitecturalInvariantsTest {
     ...
     * instead of `OpenSapienHttpClient` directly). The test is scoped
    ...
    fun `ui layer must not import http or dto types`() {
        val uiDir = File("src/main/kotlin/com/opensapien/relay/ui")
        if (!uiDir.exists()) {
        ...
        val offenders = uiDir.walkTopDown()
            ...
            .filter { f ->
                ...
                text.lineSequence().any { line ->
                    ...
                    trimmed.startsWith("import com.opensapien.relay.http")
                }
            }
        ...
        assertTrue(
            "ui/ files must not import com.opensapien.relay.http.* (INV-11 UI guard):\n  ${offenders.joinToString("\n  ")}",
            offenders.isEmpty(),
        )
    }
}
