...
import com.opensapien.relay.http.dto.CaptureEventDto
import com.opensapien.relay.http.dto.SessionDetailsDto
import com.opensapien.relay.http.dto.SessionsPageDto
import com.opensapien.relay.http.dto.toDomain
import com.opensapien.relay.relay.RelayController
import com.opensapien.relay.relay.IntentRelayStarter
import com.opensapien.relay.relay.RelayStarter
import com.opensapien.relay.store.ServerConfig
 ...
 * [com.opensapien.relay.SenseApplication.onCreate], consumed by ViewModels
 * and screens.
 *
...
/**
 * A config-aware, thread-safe provider of the current [OpenSapienHttpClient].
 * Each call reads the persisted [com.opensapien.relay.store.Config] (so a
 ...
 */
private fun clientProvider(config: ConfigurationRepository): suspend () -> OpenSapienHttpClient {
    val mutex = Mutex()
    var cached: Pair<Pair<String, String>, OpenSapienHttpClient>? = null
    return {
        ...
        mutex.withLock {
            cached?.takeIf { it.first == key }?.second
                ?: OpenSapienHttpClient(c.serverUrl, c.token).also { cached = key to it }
        }
    }
}
 ...
 */
private fun statusFetch(client: suspend () -> OpenSapienHttpClient): suspend () -> Outcome<ServerStatus> = {
    try {
 ...
 */
private class HttpSessionApi(private val client: suspend () -> OpenSapienHttpClient) : SessionApi {
