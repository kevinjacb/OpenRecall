...
import com.opensapien.relay.ble.SensorLink
import com.opensapien.relay.http.OpenSapienHttpClient
import com.opensapien.relay.setup.BleProvisioning
import com.opensapien.relay.setup.DeviceScanner
import com.opensapien.relay.setup.ProvFrames
import com.opensapien.relay.setup.ServerApi
import com.opensapien.relay.setup.SetupViewModel
...
class SetupActivity : ComponentActivity() {
    ...
    private val blePermissionLauncher = registerForActivityResult(
        ...
        if (grants.values.all { it }) {
            ...
            Toast.makeText(this, "Bluetooth permission is required to find the OpenSapien device", Toast.LENGTH_LONG).show()
        }
    }
...
object RealServerApi : ServerApi {
    override suspend fun health(urlBase: String, token: String): Boolean =
        OpenSapienHttpClient(urlBase, token).health()

    override suspend fun pubkey(urlBase: String, token: String): ByteArray =
        OpenSapienHttpClient(urlBase, token).serverPubkey()

    override suspend fun gatewayPort(urlBase: String, token: String): Int? =
        runCatching { OpenSapienHttpClient(urlBase, token).gatewayPort() }.getOrNull()
}
...
class RealDeviceScanner(private val context: Context) : DeviceScanner {
    ...
    override suspend fun scan(): List<String> {
        ...
        // The firmware advertises the AUDIO service UUID (6e9d0001) + name "OpenSapien" in its
        ...
        val filter = ScanFilter.Builder().setServiceUuid(ParcelUuid(OPENSAPIEN_SERVICE)).build()
    ...
    companion object {
        ...
        private val OPENSAPIEN_SERVICE: UUID = SensorLink.SERVICE
