...
fun DeviceRoute(onBack: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Column(
        modifier
            .fillMaxSize()
            .background(RecallTheme.colors.canvas)
            .statusBarsPadding(),
    ) {
        RecallDetailHeader(title = "Device", onBack = onBack)
        DeviceScreen(
            state = state,
 ...
 * card carrying the composite status, then [RecallGroupLabel] + [SettingsGroup]
...
fun DeviceScreen(
    ...
    PullToRefreshBox(
        ...
        Column(
            ...
            RecallGroupLabel("Server", Modifier.padding(top = 26.dp, bottom = 12.dp))
...
private fun LinkHero(relay: RelayState, onRetryConnection: () -> Unit) {
    ...
    RecallCard {
        ...
        Text(
            text = (relay.device as? DeviceState.Connected)?.name ?: "OpenRecall",
 ...
 * all to "unreachable".
 *
 * This INTENTIONALLY diverges from [com.openrecall.relay.core.ui.toDisplayMessage]
