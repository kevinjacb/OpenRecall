 ...
 * [com.openrecall.relay.RecallApplication.onCreate], consumed by ViewModels
 * and screens.
 *
...
/**
 * A config-aware, thread-safe provider of the current [OpenRecallHttpClient].
 * Each call reads the persisted [com.openrecall.relay.store.Config] (so a
 ...
 */
private fun clientProvider(config: ConfigurationRepository): suspend () -> OpenRecallHttpClient {
    val mutex = Mutex()
    var cached: Pair<Pair<String, String>, OpenRecallHttpClient>? = null
    return {
        ...
        mutex.withLock {
            cached?.takeIf { it.first == key }?.second
                ?: OpenRecallHttpClient(c.serverUrl, c.token).also { cached = key to it }
        }
    }
}
 ...
 */
private fun statusFetch(client: suspend () -> OpenRecallHttpClient): suspend () -> Outcome<ServerStatus> = {
    try {
