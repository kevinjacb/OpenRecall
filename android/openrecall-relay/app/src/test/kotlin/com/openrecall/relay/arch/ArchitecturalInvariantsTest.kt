package com.openrecall.relay.arch
 ...
 * anything from `com.openrecall.relay.http` or `com.openrecall.relay.http.dto`.
 *
...
class ArchitecturalInvariantsTest {
     ...
     * instead of `OpenRecallHttpClient` directly). The test is scoped
    ...
    fun `ui layer must not import http or dto types`() {
        val uiDir = File("src/main/kotlin/com/openrecall/relay/ui")
        if (!uiDir.exists()) {
        ...
        val offenders = uiDir.walkTopDown()
            ...
            .filter { f ->
                ...
                text.lineSequence().any { line ->
                    ...
                    trimmed.startsWith("import com.openrecall.relay.http")
                }
            }
        ...
        assertTrue(
            "ui/ files must not import com.openrecall.relay.http.* (INV-11 UI guard):\n  ${offenders.joinToString("\n  ")}",
            offenders.isEmpty(),
        )
    }
}
