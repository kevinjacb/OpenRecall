...
import com.openrecall.relay.data.AgentOutcomeKind
...
class ChatViewModelTest {
    ...
    fun `ask sends user message and receives answer`() = runTest(dispatcher) {
        val repo = FakeAgentRepo { sessionId, text ->
            AgentOutcome.Answer(
                ...
                trace = com.openrecall.relay.core.TraceContext("r1", "t1", "a1"),
            )
        }
    ...
    fun `ask with refuse outcome renders refusal message`() = runTest(dispatcher) {
        val repo = FakeAgentRepo { _, _ ->
            AgentOutcome.Refuse(
                ...
                trace = com.openrecall.relay.core.TraceContext("r1"),
            )
        }
        val store = ChatHistoryStore()
        val vm = ChatViewModel(repo, store)
    ...
    fun `clear empties the history`() = runTest(dispatcher) {
        val repo = FakeAgentRepo { _, _ ->
            AgentOutcome.Refuse("x", com.openrecall.relay.core.TraceContext("r"))
        }
        val store = ChatHistoryStore()
        val vm = ChatViewModel(repo, store)
        vm.onTextChanged("hi")
...
private class FakeAgentApiForRepo : com.openrecall.relay.data.AgentApi(
    baseUrl = "http://test",
    token = "t",
) {
    override suspend fun postAgent(
        ...
        text: String,
        limit: Int,
    ): com.openrecall.relay.http.dto.AgentResponseDto = error("not used in chat vm tests")
}
