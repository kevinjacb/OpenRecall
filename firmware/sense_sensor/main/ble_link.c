...
#define SENSE_UUID128(variant)                                                        \
  BLE_UUID128_INIT(0x10, 0x0a, 0x8f, 0x5e, 0x2d, 0x7c, 0x1a, 0x9b, 0x6e, 0x4f, 0xa3, \
                   0xb5, (variant), 0x00, 0x9d, 0x6e)

static const ble_uuid128_t s_svc_uuid = SENSE_UUID128(0x01);
static const ble_uuid128_t s_audio_uuid = SENSE_UUID128(0x02);
...
static void start_advertising(void) {
  ...
  if (rc != 0) {
    ...
    ESP_LOGI(TAG, "advertising as \"Sense\"");
  }
}
...
esp_err_t ble_link_start(ble_command_handler_t on_command) {
  ...
  if (rc != 0) return ESP_FAIL;
  if (ble_svc_gap_device_name_set("Sense") != 0) return ESP_FAIL;
